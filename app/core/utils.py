...
def find_temp_range(input_path):
    """
    поиск минимального и максимального значения температуры в отображаемом на клиенте файле 
    """
    ...
    with rasterio.open(input_path) as src:
 	...
 	#2.1 Находим правую границку (колово пикселей равное 10% от максимума)
        ...
        temp_range = {'min_temp': left_ridge, 'max_temp': right_ridge}
        return temp_range
...
def detect_temperature_band(src):
    ...
    return sorted(candidates, key=lambda x: x['unique'], reverse=True)[0]['band'] if candidates else 1

    
def get_min_max_temp(tiff_path: str) -> dict:
    """
    Возвращает минимальную и максимальную температуру из TIFF-файла.
    
    Args:
        tiff_path (str): Путь к TIFF-файлу с температурными данными.
    
    Returns:
        dict: Словарь с ключами 'min_temp' и 'max_temp' или None, если данные не найдены.
    """
    ...
    try:
        with rasterio.open(tiff_path) as src:
            ...
            band_data = src.read(band_number)
            valid_data = band_data[~np.isnan(band_data) & (band_data != src.nodata)]
            
            if valid_data.size == 0:
                print("Нет валидных температурных данных в файле")
                return None
