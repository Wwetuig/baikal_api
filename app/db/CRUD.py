...
from db.models import Data_type, Measuring_devices, Measured_parameters, User, Coordinates, Units_measurement, EmailVerification
...
from core.utils import get_temperature_list_with_coordinates, find_temp_range
...
async def get_current_active_user_email(token: str = Depends(oauth2_scheme), db: AsyncSession = Depends(get_db)):
...
async def create_verification_request(request: EmailVerification, db: AsyncSession = Depends(get_db)):
    # Добавляем пользователя в базу данных
    db.add(request)
    await db.commit()
    return request
...
async def verify_user(user: User, db: AsyncSession = Depends(get_db)):
    user.roles_id = 2
